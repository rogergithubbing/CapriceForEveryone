...
#ifdef __WIN32__
static const tULong CRC32_Table[] =
{
...
tULong CRC32(const tUChar* Data,
             tULong Length)
...
{
#ifdef __PALMOS__
tCRC32Param* paramP = (tCRC32Param*)userData68KP;
#endif /* __PALMOS__ */
tULong crc32 = 0xffffffff;
const tUChar* bufferP;
const tULong* crctableP;
tULong size;

#if defined(__PALMOS__)
  // Palm OS 68K interface
  bufferP = (tUChar*)EndianSwap32(paramP->bufferP);
  size = (tULong)EndianSwap32(paramP->size);
  crctableP = (tULong*)EndianSwap32(paramP->crc32tableP);
#else /* __PALMOS__ */
  bufferP = Data;
  size = Length;
  crctableP = CRC32_Table;
#endif /* __PALMOS__ */

  while (size--)
  {
    crc32 = (crc32 >> 8) ^ crctableP[(crc32 & 0xFF) ^ *(bufferP++)];
  }
